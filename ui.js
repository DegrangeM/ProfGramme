	...
	updateHolidays: function (i) {
		...
		Holidays.forEach(function (h) {
			if (start <= h && h <= end) {
				let d = new Date();
				d.setTime(h);
				block.find('.items').prepend('<span class="holydays">' + d.toLocaleDateString('default', {
						weekday: 'long',
						day: '2-digit',
						month: 'long'
					}) + '</span>');
			...
			Timeline.forEach(function (timeline, i) {
				...
				WeekHolidays.forEach(function (h) {
					if (start < h.time && h.time < end) {
						...
						block.find('.items').prepend('<span class="weekholydays">' + new Date(h.time).toLocaleDateString('default', {
								day: '2-digit',
								month: 'long'
							}) + ' → ' + new Date(h.time + h.duration * 1000 * 60 * 60 * 24 * 7).toLocaleDateString('default', {
								day: '2-digit',
								month: 'long'
							}) + ' (' + h.duration + (h.duration > 1 ? ' semaines' : ' semaine') + ')</span>');
					}
				});
